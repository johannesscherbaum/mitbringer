...
export const api = {
  ...
  requests:      (lat?: number, lng?: number, all?: boolean, withAssigned?: boolean) => { const p = new URLSearchParams(); if (lat != null) p.set('lat', String(lat)); if (lng != null) p.set('lng', String(lng)); if (all) p.set('all', '1'); if (withAssigned) p.set('assigned', '1'); return req<any[]>('GET', `/requests${p.toString() ? '?' + p : ''}`); },
